        ...
        stage('Run the K8s Example') {
             dir('Palisade-examples') {
                 container('helm') {
                     ...
                     sh("kubectl delete pod,pvc,sc --namespace=${GIT_BRANCH_NAME_LOWER} --all", returnStatus=false)
                     sh("kubectl delete ns ${GIT_BRANCH_NAME_LOWER}", returnStatus=false)
                     sh("kubectl delete pv palisade-classpath-jars-example-${GIT_BRANCH_NAME_LOWER}", returnStatus=false)
                     sh("kubectl delete pv palisade-data-store-${GIT_BRANCH_NAME_LOWER}", returnStatus=false)
