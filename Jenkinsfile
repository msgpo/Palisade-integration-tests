...
''') {
    node(POD_LABEL) {
        ...
        stage('Run the JVM Example') {
                // Always run some sort of smoke test if this is a Pull Request or from develop or master
                if (env.BRANCH_NAME.substring(0, 2) == "PR" || env.BRANCH_NAME == "develop" || env.BRANCH_NAME == "master") {
